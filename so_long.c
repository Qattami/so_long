/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   so_long.c                                          :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: iqattami <iqattami@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/16 14:35:31 by iqattami          #+#    #+#             */
/*   Updated: 2024/07/15 01:12:45 by iqattami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

int protection_1(char **tab, int len)
{
    if(!tab)
        return (write(1, "Error\n", 6), exit(1), 1);
    if(line_len(tab, len) == 1)
        return (write(1, "Error\n", 6), exit(1), 1);
    if(check_border(tab, len) == 1)
        return (write(1, "Error\n", 6), exit(1), 1);   
    if(out_of_map(tab, len - 1))
        return (write(1, "Error\n", 6), exit(1), 1);
    if(rectangular(tab, len - 1))
        return(write(1, "Error\n", 6), exit(1), 1);
    return(0);
}
void	ft_str_replace(char *str, char c1, char c2)
{
	int	i;

	i = 0;
	if (!str)
		return ;
	while (str[i])
	{
		if (str[i] == c1)
			str[i] = c2;
		i++;
	}
}
int	exit_game(s_data *app)
{
    free_map(app->map);
	ft_print_error("You exit the game 🙄", app, 0);
    exit(0);
	return (0);
}
int get_len(char *str)
{
    int len;
    int c;
    char *tmp;

    len = 0;
    c = open(str, O_RDONLY);
    if(c < 1)
        exit(1);
    tmp = get_next_line(c);
    if(tmp == NULL)
    {
        write(1,"empty map\n", 10);
        exit(1);
    }
        
    while (tmp)
    {
        len++;
        free(tmp);
        tmp = get_next_line(c);
    }
    close(c);
    free(tmp);
    return (len);
}
char **get_tab(char *str, int len)
{
    int c;
    int i;
    char **tab;

    i = 0;
    tab = malloc(sizeof(char *) * (len + 1));
    if(!tab)
        exit(1);
        
    c = open(str, O_RDONLY);
    if(c < 1)
        exit(1);
    while (len > i)
    { 
        tab[i] = get_next_line(c);
        ft_str_replace(tab[i++], '\n', '\0');
    }
    tab[i] = NULL;
    return (tab);
}
int main(int ac, char **av)
{
    if(ac == 2)
    {
        s_data app;
        int len;
        char **tab;
                
        len = get_len(av[1]);
        tab = get_tab(av[1], len);
        app.map = get_tab(av[1], len);
        protection_1(tab, len);
        ft_validate_path(&app, len);
        if(in_map(&app))
            ft_print_error("error\n", &app, 1);
         app.mlx = mlx_init();
         load_images(&app);
         app.win = mlx_new_window(app.mlx, ft_strlen(app.map[0]) * 30, len * 30, "so_long!");
         draw_map(&app);
         app.moves = 0;
        mlx_hook(app.win, 17, 0, exit_game, &app);        
        mlx_hook(app.win, 02, 1L<<0, handle_keys, &app);
        mlx_loop(app.mlx);
        mlx_destroy_display(app.mlx);
    }

}



