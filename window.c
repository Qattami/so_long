...
/*   Updated: 2024/06/14 02:53:03 by iqattami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void load_images(s_data *point)
{
    int h;
    int w;
    point->player = mlx_xpm_file_to_image(point->mlx, "bubu0.xpm", &h, &w);
    if(!(point->player))
        ft_print_error("player can't load \n", point, 1);
    point->wall = mlx_xpm_file_to_image(point->mlx, "wall.xpm", &h, &w);
    if(!(point->wall))
        ft_print_error("wall can't load \n", point, 1);
    point->burger = mlx_xpm_file_to_image(point->mlx, "burger.xpm", &h, &w);
    if(!(point->burger))
        ft_print_error("burger can't load \n", point, 1);
    point->ground = mlx_xpm_file_to_image(point->mlx, "wall.xpm", &h, &w);
    if(!(point->ground))
        ft_print_error("ground can't load \n", point, 1);
    point->exit = mlx_xpm_file_to_image(point->mlx, "wall.xpm", &h, &w);
    if(!(point->exit))
