...
/*   Updated: 2024/06/25 12:02:22 by iqattami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void load_images(s_data *point)
{
    ...
    point->player = mlx_xpm_file_to_image(point->mlx, "images/player.xpm", &h, &w);
    if(!point->player)
    {
        printf("----%p\n",point->player);
        ft_print_error("player can't load \n", point, 1);
    }
    point->wall = mlx_xpm_file_to_image(point->mlx, "images/wall.xpm", &h, &w);
    if(!(point->wall))
        ft_print_error("wall can't load \n", point, 1);
    point->burger = mlx_xpm_file_to_image(point->mlx, "images/kk.xpm", &h, &w);
    if(!(point->burger))
